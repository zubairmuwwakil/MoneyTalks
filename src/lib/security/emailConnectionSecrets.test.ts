...
it("never stores a secret under the wrong column's binding", () => {
  const out = encryptConnectionSecrets(USER, { accessToken: "value", refreshToken: "value" });
  ...
  expect(readConnectionSecret(USER, "accessToken", String(out.refreshToken))).toBe(null);
});

it("reads back a secret written for the same user and column", () => {
  const stored = encryptSecret("s3cret", { userId: USER, field: "refreshToken" });
  expect(readConnectionSecret(USER, "refreshToken", stored)).toBe("s3cret");
});
