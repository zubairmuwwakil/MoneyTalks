...
/**
 * The CSV column-mapping fields shared by every CSV upload form: column
 * indexes are 0-based positions into each parsed CSV row. Consumers that
 * don't need a transaction-type sign convention (e.g. the statement
 * analyzer) validate against this directly instead of the full
 * `csvImportInput`, which also carries the type-select fields below.
 */
export const csvMappingInput = z.object({
 ...
 * form. `positiveType`/`negativeType` decide the resulting `TxType` from
 * the sign of the mapped amount.
 */
export const csvImportInput = csvMappingInput.extend({
