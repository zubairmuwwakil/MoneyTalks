...
function rewriteBlock(sectionId: string, block: Block): Block {
  ...
  if (sectionId === "on-device" && block.kind === "p" && block.text.startsWith("The shopping record described above — merchants, recommendations, purchase entries, and corrections — never leaves your device.")) {
    return {
      kind: "p",
      text: "The shopping record described above — merchants, recommendations, purchase entries, corrections, and local MCC-learning evidence — never leaves your device as account data. If you choose Import issuer MCC CSV, PickMe reads that file locally. It may compare the imported merchant, date, CAD amount, and card network transiently against purchases already on this iPhone so a literal MCC can be attached to one store location only when exactly one purchase matches. Those matching fields are not copied into the MCC-learning ledger or sent to In Unity. PickMe keeps only normalized merchant, literal MCC, optional network, observation date and, after a safe unique match, the already-local store coordinates; it does not retain the raw issuer file, imported purchase amount, card/account number, filename, or statement text. When you sign in, the separate wallet configuration listed under Your cards and Your card settings does sync so the server can evaluate Wallet Shortcut captures. Optional community evidence is narrower and separate: only the specific gift-card or literal-MCC fields described below can be shared anonymously when you enable those settings.",
    };
  }
